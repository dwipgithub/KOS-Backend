...
const uploadRoot = path.join(__dirname, "..", "uploads", "pemasukan")
...
export function pemasukanBuktiUpload(req, res, next) {
    const ct = req.headers["content-type"] || ""
    if (!ct.includes("multipart/form-data")) {
        return response.error(
            res,
            new Error("Gunakan multipart/form-data dan unggah bukti pemasukan"),
            400
        )
    }

    return upload.single("buktiPemasukan")(req, res, (err) => {
        if (err) {
        ...
        if (!req.file) {
            return response.error(res, new Error("Bukti pemasukan wajib diunggah"), 400)
        }
        return next()
    })
}
